    ...
    <div class="flex-1 bg-purple-400 p-8 overflow-auto">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-purple-800 text-white">
                        ...
                        <th class="py-3 px-4 text-left">Compte</th>
                ...
                <tbody>
                    ...
                    <tr class="border-b border-gray-400 hover:bg-gray-200 transition-colors">
                        <td class="py-3 px-4">
                            <div class="flex items-center">
                                <div class="w-10 h-10 bg-purple-800 rounded-full flex items-center justify-center text-white mr-3">
                                    <span>{{ substr($transaction->nom, 0, 1) }}</span>
                                </div>
                                <span>{{ $transaction->nom }}</span>
                            </div>
                        </td>
                        <td class="py-3 px-4">{{ $transaction->date->format('d/m/Y') }}</td>
                        <td class="py-3 px-4 {{ $transaction->montant_color }} font-semibold">
                            {{ $transaction->montant_formatte }}
                        </td>
                        <td class="py-3 px-4">
                        ...
                        <td class="py-3 px-4">{{ $transaction->compte->nom_compte }}</td>
                        <td class="py-3 px-4">
                            <div class="flex space-x-2">
                                <a href="{{ route('transactions.show', $transaction->id) }}" class="text-blue-600 hover:text-blue-800">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                    </svg>
                                </a>
                                <a href="{{ route('transactions.edit', $transaction->id) }}" class="text-yellow-600 hover:text-yellow-800">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </a>
                                <form method="POST" action="{{ route('transactions.destroy', $transaction->id) }}" class="inline">
                                    @csrf
                                    @method('DELETE')
                    ...
                    <tr>
                        <td colspan="6" class="py-8 text-center text-gray-500">
                            <p class="text-lg mb-4">Aucune transaction trouvée.</p>
                            <a href="{{ route('transactions.create') }}" class="bg-purple-800 text-white px-4 py-2 rounded-lg hover:bg-purple-700 transition-colors">
        ...
        <div class="mt-6 flex justify-center">
            <nav class="flex items-center space-x-2">
                <span class="text-white">{{ $transactions->count() }} transaction(s) trouvée(s)</span>
            </nav>
        </div>
        @endif
    </div>
